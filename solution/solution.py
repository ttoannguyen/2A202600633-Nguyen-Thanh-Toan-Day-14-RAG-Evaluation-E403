...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            chunk_score = len(chunk_tokens & expected_tokens) / len(expected_tokens)
            relevant_flags.append(chunk_score >= relevance_threshold)
...
def rerank_by_expected_coverage(contexts: list[str], expected: str) -> list[str]:
    """Custom reranker (Exercise 3.5, Bước 6) — improves on `rerank_by_overlap`
    in two ways:

    1. Scores against the EXPECTED ANSWER tokens, not the query tokens. This
       lines up exactly with how `evaluate_context_precision` itself decides
       "relevant" (overlap with `expected`), so it optimizes the metric more
       directly than query-overlap does — query wording and answer wording
       often diverge (paraphrase gap), which caps how high a query-only
       reranker can push precision.
    2. Length-penalizes the score: long chunks rack up overlap just by
       containing more words, so raw overlap count != is more dense in
       useful evidence. Dividing by chunk length (+1 to avoid div-by-zero)
       favors concise, evidence-dense chunks over padded ones.

    score(chunk) = |tokenize(chunk) ∩ tokenize(expected)| / (|tokenize(chunk)| + 1)

    Caveat: at real inference time the expected/ground-truth answer is not
    available (only the query is) — this reranker is an "oracle" upper bound
    useful for offline analysis/labeling, not a drop-in production reranker.
    """
    expected_tokens = _tokenize(expected)
    ...
    def _score(chunk: str) -> float:
        ...
        return overlap / (len(chunk_tokens) + 1)
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric:\n{rubric_lines}\n"
            f"Return a JSON object mapping each criterion to a score from 0 to 1."
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for entry in scores_batch:
            ...
            avgs.append(sum(values) / len(values) if values else 0.0)
        ...
        positional_bias = False
        if len(avgs) >= 2:
            first, rest = avgs[0], avgs[1:]
            rest_avg = sum(rest) / len(rest)
            positional_bias = (first - rest_avg) > 0.1
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            result.qa_pair = pair  # keep original qa_pair (with metadata)
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        key = low[0] if low else min(scores, key=scores.get)
        mapping = {
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, f in enumerate(failures):
            ...
            fix = suggestions[i] if i < len(suggestions) else (
                suggestions[-1] if suggestions else "Review pipeline"
            )
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant"):
            suggestions.append("Add few-shot examples and clarify prompt to keep answers on-topic")
        ...
        if categories.get("off_topic"):
            suggestions.append("Improve intent detection to route to the correct skill")
        if categories.get("refusal"):
            suggestions.append("Relax overly strict guardrails for legitimate in-scope queries")

        fallback = [
            "Increase chunk size in RAG pipeline to reduce context fragmentation",
            "Add few-shot examples showing complete answers to improve completeness",
            "Implement hallucination checker to filter unsupported claims",
            "Improve retrieval ranking to surface relevant context first",
            "Review prompt clarity to reduce irrelevant answers",
        ]
        i = 0
        while len(suggestions) < 3 and i < len(fallback):
            if fallback[i] not in suggestions:
                suggestions.append(fallback[i])
            i += 1
